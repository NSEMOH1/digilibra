...
class DBHelper {
  ...
  initDb() async {
    ...
    String path = join(documentsDirectory.path, 'libra.db');
  ...
  Future<bool> setAvatarForContact(Contact contact, String avatarPath) async {
    var dbClient = await db;
    return await dbClient.rawUpdate(
            'UPDATE contacts SET avatar_path = ? WHERE address = ?',
            [avatarPath, contact.address]) >
        0;
  }
